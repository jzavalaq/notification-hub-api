...
class WebhookServiceTest {
    ...
    void getUserWebhooks_existingUser_returnsList() {
        ...
        when(webhookRepository.findByUserId(eq("user-123"), any(Pageable.class))).thenReturn(page);

        PageResponse<WebhookDto.Response> response = webhookService.getUserWebhooks("user-123", 0, 20);
    ...
    void getUserWebhooks_userWithNoWebhooks_returnsEmptyList() {
        Page<WebhookEndpoint> emptyPage = new PageImpl<>(List.of());
        when(webhookRepository.findByUserId(eq("nonexistent"), any(Pageable.class))).thenReturn(emptyPage);

        PageResponse<WebhookDto.Response> response = webhookService.getUserWebhooks("nonexistent", 0, 20);
